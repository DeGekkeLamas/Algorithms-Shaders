...
namespace MovementStuff
{
    ...
    public class WASDMovement : MovingObjectBase
    {
        ...
        void Update()
        {
            ...
            if (!Physics.Raycast(this.transform.position + this.transform.localScale.y * .75f * Vector3.up, 
                movement, moveSpeed * baseSpeed * Time.deltaTime + this.transform.lossyScale.z * .5f))
            {
